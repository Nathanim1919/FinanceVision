...
export const registerUser = asyncHandler(async (req, res) => {
  ...
  const chatBoard = await Chat.create({user : user._id})

  user.chatBoard = chatBoard._id;

  /**
   * unHashedToken: unHashed token is something we will send to the user's mail
   * hashedToken: we will keep record of hashedToken to validate the
   * unHashedToken in verify email controller tokenExpiry: Expiry to be checked
   * before validating the incoming token
   */
...
export const verifyEmail = asyncHandler(async (req, res) => {
  ...
  // While registering the user, same time when we are sending the verification
  // mail we have saved a hashed value of the original email verification token
  // in the db We will try to find user with the hashed token generated by
  // received token If we find the user another check is if token expiry of that
  // token is greater than current time if not that means it is expired
  ...
  // Now we can remove the associated email token and expiry date as we no
  // longer need them
...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    // attach refresh token to the user document to avoid refreshing the
    // accesstoken with multiple refresh tokens
...
export const forgotPasswordRequest = asyncHandler(async (req, res) => {
  ...
  // Send mail with the password reset link, It should be the link of the
  // frontend url with token

  await sendEmail({
    email : user.email,
    Subject : "Reset Password",
    text : `If you have sent this reset password request, Click the like http://localhost:5173/resetPassword/${
        unHashedToken} <br/> but if this request is not from you, you dont have to anything!`
  })

  return res.status(200).json(new ApiResponse(
      200, {}, "Password reset mail has been sent on your email"))
});
...
export const resetForgottenPassword = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    res.status(489).json({message : "Token is invalid or expired"})
  }
