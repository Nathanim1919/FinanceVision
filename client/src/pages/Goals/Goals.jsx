...
export const Goals = () => {
  ...
  return isLoading ? (
    ...
    <Content>
      {showDetails && selectGoal && (
        <GoalDetails setShowDetails={setShowDetails} goal={selectGoal} />
      )}
            ...
            goals.map((item) => (
              <Card key={item.title}>
                <div className="titles">
                  <h4>
                    <TbCategoryFilled />
                    {item.title.slice(0, 15)}..<span>{item.category}</span>
                  </h4>
                  <div className="current-progress">
                    <p className="daysleft">
                      {item.progress === 100
                        ? "Completed"
                        : calculateTimeLeft(item.startDate, item.deadline)}
                    </p>
                    <h2>
                      <GoGoal />
                      {formatNumber(item.target)} ETB
                    </h2>
                  </div>
                </div>
                  ...
                  <div>
                    <div className="outter">
                      <div
                        style={{
                          backgroundColor:
                            item.progress === 100 ? "#41cc87" : "#1b76ff",
                          width: `${item.progress}%`,
                        }}
                        className="inner"
                      ></div>
                ...
                <div className="icons">
                  ...
                  <div
                    ...
                    onClick={() =>
                      dispatch(
                        deleteGoal({ userId: user._id, goalId: item._id }),
                      )
                    }
                  >
                    <MdDeleteOutline />
  ...
  .progress {
    ...
    .inner {
      ...
      background-color: ${(props) =>
        props.progress === 100 ? "green" : "#1b76ff"};
      position: absolute;
      left: 0;
    }
  }
